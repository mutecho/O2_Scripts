...
void CFCalc_inclusive(string rpath, string rfilename, string taskname,
  ...
  // histo path inside root file
  string hpath_se = taskname + "/" + subtaskname_se + "/relPairDist";
  string hpath_me = taskname + "/" + subtaskname_me + "/relPairDist";

  //   TH1D *h1_se = onefromndHisto(rf, hpath_se, false, 0);
  //   TH1D *h1_me = onefromndHisto(rf, hpath_me, false, 0);

  cout << "Starting CF calculation..." << endl;
  string hname = "inclusive";
  string hname_me = hname + "/" + "ME";
  TH1D *h1_me = (TH1D *)rf->Get(hpath_me.c_str());
  // TH1D *h1_me = ndHistoRead(rf, hpath_me, centLow, centHigh, mTLow,
  // mTHigh,epBins_me[0]);
  if (!h1_me) {
    cout << "ERROR: ME histo not found: " << hpath_me << endl;
    continue;
  }
  ...
  string hname_se = hname + "/SE";
  // auto h1_se = (TH1D *)rf_se->Get(hname_se.c_str());
  // TH1D *h1_se = ndHistoRead(rf, hpath_se, centLow, centHigh, mTLow,
  // mTHigh,epBins_me[0]);
  TH1D *h1_se = (TH1D *)rf->Get(hpath_se.c_str());
  if (!h1_se) {
    cout << "ERROR: ME histo not found: " << hname_me << endl;
    continue;
  }
...
void get_cf_from_exp() {
  // string suffix = "_02ptmin";
  // string suffix = "_its";
  string suffix = "_2400bins";
  // string rpath1 = "/Users/allenzhou/ALICE/alidata/femtoep_res";
  // string rpath1 = "/Users/allenzhou/ALICE/alidata/hyperloop_res/femtoep";
  // string data_set = "23zzo_pass5_medium";
  // string data_set = "23zzh_pass5_small_dbgt4";
  // string data_set = "23zzh_pass5_morebin";
  string data_set = "25ae_pass2";
  string rpath = "/Users/allenzhou/ALICE/alidata/hyperloop_res/femtoep/OO";
  ...
  string rtaskname = "femto-dream-pair-task-track-track";
  // string rtaskname = "femto-dream-pair-task-track-track" + suffix;
  ...
  // string wfilename1 = "EP_dependence_CF_" + data_set + suffix;
